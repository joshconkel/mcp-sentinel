...
class LMStudioBackend(LLMBackend):
    """
    LMStudio local API (OpenAI-compatible). No extra dependencies.

    Qwen 3.x and other reasoning models emit a <think>...</think> block
    before the actual response. This can exhaust max_tokens and leave the
    content field empty. Three mitigations are applied here:

      1. Thinking is disabled by default via chat_template_kwargs so the
         model goes straight to the JSON response. Pass thinking=True to
         re-enable (useful for debugging or non-Qwen models).
      2. max_tokens raised to 4096 to give budget for both thinking (if
         enabled) and the JSON response.
      3. <think> blocks are stripped from content, and reasoning_content
         is used as a fallback if content is empty.
    """
    ...
    @property
    def name(self) -> str:
        think_label = "+think" if self.thinking else "no-think"
        return f"LMStudio ({self.host}, {think_label})"
    ...
    def complete(self, system: str, user: str) -> str:
        ...
        # Disable extended thinking for structured JSON generation.
        # Qwen 3.x honours this via chat_template_kwargs; other models ignore it.
        ...
        # Primary: content field (normal and thinking-in-content modes)
        content = (msg.get("content") or "").strip()

        # Strip <think>...</think> blocks that precede the JSON response
        content = re.sub(r"<think>.*?</think>", "", content, flags=re.DOTALL).strip()

        # Fallback: some Qwen 3 + LMStudio versions route thinking to
        ...
        if not content:
            raise RuntimeError(
                "Model returned empty content. The thinking tokens may have "
                f"consumed the entire max_tokens budget ({self.max_tokens}). "
                "Try: --max-tokens 8192, or pass --thinking to debug the raw output."
            )
...
def main() -> None:
    ...
    parser.add_argument("--max-tokens", type=int, default=4096, metavar="N",
                        help="Max tokens for LLM response (default: 4096). "
                             "Raise to 8192 if Qwen 3 thinking exhausts budget.")
    parser.add_argument("--thinking", action="store_true",
                        help="Enable extended thinking for Qwen 3 and other "
                             "reasoning models. Disabled by default for faster, "
